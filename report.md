...
This graph shows the effect of changing the block size on the speed of Flash Attention. $B_r$ is the height of the block (and the axis where we parelelize) and $B_c$ is the width of the block which obviously effects the total size of the block as well as the SRAM utilization. You can see that as $B_r$ increases the implementation gets faster (up to a point), this makes sense as the profiler results suggest that we are compute bound and have relatively low thread utilization, thus spawning more threads to do work on the blocks speed up the computation. 
...
Sequence length scaling before parelelizing over $Q_i$
![Sequence length scaling](timing_plots_before_qi/seq_len.png)

Sequence length scaling after parelelizing $Q_i$
![Sequence length scaling](timing_plots/seq_len.png)

We see that the timing scales subquadratically in both cases but that scaling is better in the case where we spawn more blocks, this makes sense because it involves more parellelism along the scaling dimention. In both cases the scaling tends towards quadratic because attention is a quadratic operation in sequence length, thus, since parellelism eventually becomes contrained by GPU resources we would expect eventual convergence to quadratic (we can see in the log-log plot that the higher points have a larger slope, closer to quadratic).

### Attention Head & Batch Size Scaling
Since both the attention head and batch size are embarassingly parallel, we would expect to see a scaling law that is roughly constant (ie ideal weak scaling). However, we see two different behaviors depending on the implementation. (For clarity the graphs will only show scaling of attention heads as it looks nearly identical to scaling for batch size.) Before implementing the optimzation that spawned threadblocks for each $Q_i$ we saw the following scaling plot.

![Embarassingly parallel before Q_i](timing_plots_before_qi/num_heads.png)

This shows roughly constant time until it jumps significantly, we hypothesize this is caused by having fewer blocks that could be maximally scheduled by the SMs, this means that adding more blocks directly creates opportunities for embarrising parellelism. However, once the number of blocks gets high enough, there are more than can be simultaneously run which means that things begen to scale linearly. This also explains why it levels off again as the number of blocks will not lead to signifcant slowdowns until it triggers another wave. The comparitively small scaling is likely because of costs of moving memory to HBM that scale loosely with problem size. 

![Embarassingly parallel after Q_i](timing_plots/num_heads.png)

After spawning blocks for each $Q_i$, there are now significantly more blocks than can be simultaneously run on the SMs, this means that as we scale up the number of blocks, parellelism doesn't increase. This creates the perfect linear scaling we see as the number of waves per SM scales with the problem size.
